...
export const GET = async (request: NextRequest) => {
	...
	const userCurrentTrainingSessionStatus =
		await prisma.userCurrentTrainingSessionStatus.findUnique({
			where: {
				userId,
			},
			select: {
	...
	if (userCurrentTrainingSessionStatus === null) {
		if (parsedBody.isActive) {
			const statusAndResult = await prisma.$transaction(async (tx) => {
				...
				const _createdSessionStatus =
					await tx.userCurrentTrainingSessionStatus.create({
						data: {
							userId,
							isActive: true,
			...
			return Response.json("Training session status is not created", {
				status: 409,
			});
	}
	if (parsedBody.isActive === userCurrentTrainingSessionStatus.isActive) {
		return Response.json("Identical training session status active", {
			status: 409,
		});
	}
		if (parsedBody.isActive) {
			const statusAndResult = await prisma.$transaction(async (tx) => {
				...
				const _updatedSessionStatus =
					await tx.userCurrentTrainingSessionStatus.update({
						where: { userId },
						data: {
							isActive: true,
							trainingSessionId: createdSession.id,
