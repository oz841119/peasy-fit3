...
import { Link, getLocaleName, routing, usePathname } from "@/i18n/routing";
...
const RowTile = ({
	...
	return (
		<div {...props} className={cn("flex items-center h-10", className)}>
			{children}
		</div>
	);
};
		...
		<Sheet open={isOpen} onOpenChange={(target) => setIsOpen(target)}>
			...
			<SheetContent side="left" className="max-w-72 sm:max-w-72 flex flex-col">
				...
				<div className="flex h-full flex-col text-sm">
					<RowTile className="font-bold text-xs text-muted-foreground">
						{t("common.pages")}
					</RowTile>
					<div>
						{menu.map((item) => (
						...
						<div className="text-sm">
							...
							{locales.map((locale) => (
								<RowTile
									...
									className={
										currLocale !== locale ? "text-muted-foreground" : ""
									}
								>
