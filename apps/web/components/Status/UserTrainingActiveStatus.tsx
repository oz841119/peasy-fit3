...
export const UserTrainingActiveStatus = ({
	size,
	showName = false,
}: UserTrainingActiveStatusProps) => {
	...
	return userTrainingSessionStatusLoading ? (
		...
		<Tooltip
			text={
				userTrainingSessionStatus?.isActive
					? t("common.inTraining")
					: t("common.notInTraining")
			}
		>
			<div className="flex items-center gap-2">
				{showName && (
					<span>{userTrainingSessionStatus?.trainingSession?.name}</span>
				)}
				<Dumbbell
					color={userTrainingSessionStatus?.isActive ? "green" : "red"}
					size={size}
